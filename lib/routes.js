(function() {
  ...
  Utils = {
    ...
    set_default_url_options: function(optional_parts, options) {
      var i, j, k, len, len1, part, ref, results;
      ...
      if (this.get_object_type(defaults.default_url_options.params) === 'object') {
        ref = Object.keys(defaults.default_url_options.params);
        results = [];
        for (i = k = 0, len1 = ref.length; k < len1; i = ++k) {
          part = ref[i];
          if (!options.hasOwnProperty(part)) {
            results.push(options[part] = defaults.default_url_options.params[part]);
          }
        }
        return results;
      }
